...
function SecretKey() {
  ...
  useEffect(() => {
    isSockdrivePremium(sockdriveEndpoint, account).then(setSockdrivePremium);
      ...
      .then(({ token, account }) => {
        if (token === knownToken) {
          ...
          if (nonSerializableStore.options.url) {
            loadBundleFromUrl(nonSerializableStore.options.url, store).catch(
              (e) => {
  ...
  return (
    <div class="mt-4 flex flex-col items-center">
      ...
      {account !== null && (
        <div class={dzMark ? "bg-warning px-2" : ""}>
          {t("hello") +
            ", " +
            (dzMark ? "DOS Zone" : account.name ?? account.email) +
            "!"}
          <span
          ...
          <>
            ...
            <span class={warnOnPremium ? "text-warning font-bold" : ""}>
              ...
              <a
                ...
                target="_blank"
                class={
                  "link ml-1 lowercase " + (warnOnPremium ? "" : "link-neutral")
                }
              >
                ({t("fix")})
              </a>
...
export function Play(props: { class?: string; button?: boolean }) {
  const configChanged = useSelector(
    (state: State) => state.editor.configChanged
  );
  ...
  async function onPlay() {
    if (configChanged) {
      ...
      try {
        ...
        dispatch(
          dosSlice.actions.bndError((e as Error).message ?? "unexpected error")
        );
      }
    } else {
      dispatch(dosSlice.actions.bndPlay({}));
  ...
  if (props.button === true) {
    return (
      <div class="btn btn-accent w-full sm:hidden" onClick={onPlay}>
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
    ...
    // custom button
    return (
      <div class={"second_play"} onClick={onPlay}>
